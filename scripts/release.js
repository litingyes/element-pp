/*
 * @Date: 2022-12-28 21:35:52
 * @Author: liting luz.liting@gmail.com
 * @LastEditors: liting luz.liting@gmail.com
 * @LastEditTime: 2022-12-29 23:11:24
 * @FilePath: /element-pp/scripts/release.js
 */
const fs = require('fs')
...
function updateDeps(pkg, depType, version) {
  ...
  Object.keys(deps).forEach((dep) => {
    if (dep === 'element-pp' || (dep.startsWith('@element-pp') && packages.includes(dep.replace(/^@element-pp\//, '')))) {
...
async function publishPackage(pkgName, version, runIfNotDry) {
  ...
  try {
    ...
    if (e.stderr.match(/previously published/)) console.log(chalk.red(`Skipping already published: ${pkgName}`))
    else throw e
  }
}
