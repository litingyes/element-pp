...
import { MILLISECOND_OF_DAY, dateAddDays, dateStartOfWeek } from '@element-pp/utils'
...
const dayScope = computed(() => {
  ...
  if (props.mode === 'week') {
    for (let i = 0; i < 7; i++) {
      const dayStart = dateAddDays(startTime.value, i).valueOf()
      const dayEnd = dateAddDays(startTime.value, i + 1).valueOf()
